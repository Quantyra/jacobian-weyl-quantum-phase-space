...
\begin{abstract}
We study the three-dimensional Keller map \(F\) announced by Alp\"oge (with Fable credited): \(\det DF\equiv -2\) and a three-point collision. Theorem~A restates those finite identities. Theorems~B--C record the standard cotangent/Piola/Weyl package for this seed; the novel analytic contribution is Theorems~D--F. The cotangent lift \(\Phi(q,p)=(F(q),J^{-T}p)\) is a polynomial Poisson map and non-injective. Dual fields \(X_j=\sum_k B_{jk}\partial_{q_k}\) commute and are divergence-free (Piola), so \(H_j=-iX_j\) are symmetric on \(C_c^\infty(\mathbb{R}^3)\). We define the induced polynomial Weyl endomorphism \(\psi\). The field \(X_1\) is incomplete via an explicit integral curve. In flow-box coordinates \((a,s,c)=F(q)\) one has \(X_1=\partial_s\) and \(\mathrm{d}q=\tfrac12\,\mathrm{d}a\,\mathrm{d}s\,\mathrm{d}c\). Open forward and backward escape walls yield whole-orbit deficiency vectors in \(\operatorname{Dom}(H^*)\) (transverse cutoffs only; no interior \(s\)-indicators), hence
\[
(n_+,n_-)=(\infty,\infty)
\]
for \(H=-iX_1\). Infinitely many self-adjoint extensions exist; the core algebraic CCR/Poisson data alone do not distinguish among them. No gates, channels, computational advantage, or seed discovery claim. Companion code candidate \textbf{v0.3.8-theorem-f} (proposed, untagged).
\end{abstract}

\noindent\textbf{MSC 2020:} 47B25, 81S05, 14R15, 37C10, 53D17.\\
\textbf{Software:} v0.3.8-theorem-f (proposed, untagged); concept DOI\\
...
\texttt{ExoticCCR.theoremE} (released tag \texttt{v0.1.8-theorem-e}, SHA
\texttt{a6bb091}): the minimal transport core for X1 is not essentially
self-adjoint via a constructed weak minus-i witness. At companion SHA
\texttt{ff50f4a2a312591c2e5b26e71eb390ade9164b34} (proposed, untagged, under review) the artifact further proves
...
finite-dimensional, the bounded reading of $(n_+,n_-)=(\infty,\infty)$, and
the standard cardinal-valued lower bounds $\operatorname{Cardinal.aleph0}\le n_+$
and $\operatorname{Cardinal.aleph0}\le n_-$. The
...
\texttt{standardDeficiencyIndex}, which is \texttt{Module.rank}
...
Neumann classification of all \texttt{SelfAdjointExtension H\_X1\_min}
witnesses (self-adjoint \texttt{LinearPMap}s extending
\texttt{H\_X1\_min}) by complex-linear isometric equivalences from the $+i$
adjoint eigenspace to the $-i$ adjoint eigenspace; it also proves at least two
distinct such self-adjoint extensions exist.

The backward-wall propositions, exact extension cardinality,
infinitely-many/continuum and algebraic-selection consequences, preferred
extension, and arbitrary-operator classification remain outside the new Lean
corollary.
Symmetry of the minimal core on $C_c^\infty$ is classical from
$\operatorname{div}X_1=0$ and is likewise not separately Lean-formalized.
...
On 19--20 July 2026, Levent Alp\"oge announced an explicit polynomial map \(F:\mathbb{C}^3\to\mathbb{C}^3\) with \(\det DF\equiv-2\) and a three-point collision, crediting Fable (and related public discussion of the problem setup)~\cite{Alpoge2026,UlamJacobian,SpeyerJC2026}. \textbf{We do not claim discovery of \(F\).} The finite seed identities are \emph{verified} here by dual CAS and by a companion Lean 4 formalization~\cite{FredriksenLean,FredriksenSoft}; independent community checks of the announcement are already circulating~\cite{SpeyerJC2026}. Our contribution is the dual-lift deficiency analysis (Theorems~D--F), not the Jacobian-conjecture refutation itself.
...
Over \(\mathbb{C}\), constant nonzero Jacobian and non-injectivity would be extraordinary for the Jacobian conjecture~\cite{BassCW}. We treat \(F\) as an externally announced seed~\cite{Alpoge2026,UlamJacobian,SpeyerJC2026}; Theorem~A is a restatement with CAS/Lean re-verification, not a priority claim. Our analytic results use the real \(L^2(\mathbb{R}^3)\) dual-field package.
\end{remark}
...
\]
is a \(C^\infty\) diffeomorphism onto an open \(X_1\)-invariant set, \(F\circ\Psi_+=(a,s,c)\), and \(s\to\beta^-\) occurs at spatial infinity. If \(\ell_+\) is finite, maximality forces escape as \(s\downarrow\ell_+\) as well. An analogous \(\Psi_-,\Omega_-\) exists for the lower wall (with \(\varepsilon_0\) inside the backward IFT window).
\end{lemma}
\begin{proof}
The maximal-flow domain \(\mathcal{D}\subset\mathbb{R}\times\mathbb{R}^3\) of the smooth vector field \(X_1\) is open and the flow \(\phi\) is \(C^\infty\) on \(\mathcal{D}\)~\cite{LeeSmooth}. The set \(D_+\) is (up to the smooth reparameterization \(s=F_1\)) the pullback of \(\mathcal{D}\) through the cross-section \(\Sigma_+\); openness of \(\mathcal{D}\) therefore yields openness of \(D_+\). Uniqueness of ODE solutions gives injectivity of \(\Psi_+\). Since \(F\circ\Psi_+=(a,s,c)\) identically on \(D_+\) and \(\det DF\neq 0\), \(\Psi_+\) is a local diffeomorphism, hence a \(C^\infty\) diffeomorphism onto its open image \(\Omega_+\). By the \(\varepsilon_0\)-window choice, the selected branch does not degenerate before the wall, so the maximal upper time equals \(\beta\). Finite lower maximal time forces escape from compacts by standard ODE maximality.
\end{proof}
...
\textit{Adjoint equation.} Let \(\varphi\in C_c^\infty(\mathbb{R}^3)\). On \(\Omega_+\), \(H=-i\partial_s\) in the flow-box chart with density \(\tfrac12\), and \(\operatorname{div} X_1=0\), so the formal adjoint of \(-iX_1\) is again \(-iX_1\). There is no interior jump because \(u_-\) has no artificial \(s\)-cutoff. Off \(\Omega_+\), \(u_-=0\). Lateral boundaries of \(W_+\) contribute nothing because \(\chi|_{\partial W_+}=0\).

For the upper end: \(s\to\beta^-\) occurs at spatial infinity, so \(\varphi\) vanishes in a neighborhood of that end along each orbit it meets. For the lower end: if \(\ell_+\) is finite then maximality forces escape to infinity as \(s\downarrow\ell_+\), and again \(\varphi\) vanishes near that end; if \(\ell_+=-\infty\), the factor \(e^{s-\beta}\) decays exponentially as \(s\to-\infty\), so both the integrated boundary term at \(-\infty\) and the \(L^2\) tail vanish (no appeal to a compact lower \(s\)-cut is needed). The Fubini/IBP interchange is justified by the integrable dominant
\[
\bigl|\chi(a,c)\,e^{s-\beta(a,c)}\,\varphi(\Psi_+(a,c,s))\bigr|
\le
\|\varphi\|_\infty\,|\chi(a,c)|\,e^{s-\beta(a,c)}
\]
on \(D_+\) (and \(0\) off \(\Omega_+\)), which is in \(L^1(D_+,\tfrac12\,\mathrm{d}a\,\mathrm{d}s\,\mathrm{d}c)\) by the same bound used for \(\|u_-\|_2\). Integrating by parts in \(s\) for almost every \((a,c)\) therefore yields
\[
...
\end{theorem}
\begin{proof}
Forward and backward walls, saturation, and the deficiency lemma. Separability of \(L^2(\mathbb{R}^3)\) makes both Hilbert-space dimensions (orthonormal-basis cardinals) countably infinite. This is a Hilbert dimension statement, not an assertion that the algebraic Hamel rank is \(\operatorname{aleph0}\).
\end{proof}
...
\section{Non-claims}
No unitary gate, channel, CP map, or computational advantage. No preferred physical extension. No claim that all quantization routes fail. Discovery of \(F\) not claimed. Lean: bounded canonical-core Theorems E--F, the exact Hilbert-basis cardinal result, the specific \texttt{H\_X1\_min} von Neumann classification, and existence of at least two distinct extension witnesses are covered at the updated SHA. The algebraic \texttt{standardDeficiencyIndex}/\texttt{Module.rank} has lower bounds only; the paper-grade backward-wall construction, exact extension cardinality, infinitely-many/continuum and algebraic-selection consequences, arbitrary-operator theorem, strong CCR, and physical selection remain outside the new Lean corollary. No theorem-level dual-flow strong CCR obstruction in this note.
...
\bibitem{Alpoge2026}
L.~Alp\"oge, informal public announcement of a Keller map with constant Jacobian and three-point collision (credits Fable), July 2026,
\url{https://x.com/__alpoge__/status/2079028340955197566}.

\bibitem{UlamJacobian}
Ulam, \emph{A Counterexample to the Jacobian Conjecture} (public write-up of the July 2026 announcement),
...
GitHub Quantyra/jacobian-weyl-quantum-phase-space, proposed candidate
v0.3.8-theorem-f (untagged);
...
released tag v0.1.8-theorem-e, SHA a6bb091 (bounded Theorem E),
...
Theorem F supplement: \texttt{ExoticCCR.theoremF},
\texttt{aleph0\_le\_standardDeficiencyIndex\_X1}, and
\filepath{hilbertDeficiencyIndex\_X1\_eq\_aleph0},
\texttt{theoremFVonNeumannClassification}, and
\texttt{theoremF\_exists\_two\_distinct\_selfAdjointExtensions} at SHA
ff50f4a2a312591c2e5b26e71eb390ade9164b34 (untagged review). Both standard
adjoint eigenspaces are not finite-dimensional, and
\(\operatorname{Cardinal.aleph0}\le n_+\) and
\(\operatorname{Cardinal.aleph0}\le n_-\). The separate Hilbert-basis
cardinal is exactly \(\operatorname{aleph0}\) for both closed eigenspaces;
\texttt{standardDeficiencyIndex} is the algebraic (Hamel) rank and its exact
...
isometric equivalences from the $+i$ to the $-i$ adjoint eigenspace, and at
least two distinct witnesses exist. No exact extension cardinality, preferred
extension, arbitrary-operator theorem, backward-wall construction, or promotion
of the paper's infinitely-many/continuum and algebraic-selection consequences
is claimed.
\end{thebibliography}
\end{document}
