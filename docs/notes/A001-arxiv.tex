...
(n_+,n_-)=(\infty,\infty)
\]
for \(H=-iX_1\). Infinitely many self-adjoint extensions exist; the core algebraic CCR/Poisson data alone do not distinguish among them. No gates, channels, computational advantage, or seed discovery claim. Companion code release \textbf{v0.3.6-submit}.
\end{abstract}

\noindent\textbf{MSC 2020:} 47B25, 81S05, 14R15, 37C10, 53D17.\\
\textbf{Software:} v0.3.6-submit; concept DOI 10.5281/zenodo.21474351 (project-level; version DOI pending Zenodo ingest of this tag).\\
...
\end{theorem}
\begin{proof}
Expand \(\det J\) as a polynomial identity, or evaluate the formal determinant in a computer algebra system; formalized in Lean 4 in the companion repository (\(\mathtt{jacobianDet\_F}\), \(\mathtt{evalMap\_F\_p0/p1/p2}\))~\cite{FredriksenLean}. Non-injectivity is immediate from \eqref{eq:collision}.
\end{proof}
...
=\text{the first-order operator associated to }[X_i,X_j]=0.
\]
...
\]
on a relatively compact \(W_-\Subset U_-\) near the wall. Regression: \texttt{data/anchor/cas\_backward\_incomplete\_wall\_A001.json}~\cite{FredriksenSoft}.
\end{proof}
...
D.~E.~Fredriksen, EXOTIC-CCR A001 companion code and CAS certificates, GitHub \texttt{Quantyra/jacobian-weyl-quantum-phase-space}, release \texttt{v0.3.6-submit},
\url{https://github.com/Quantyra/jacobian-weyl-quantum-phase-space/releases/tag/v0.3.6-submit};
concept DOI 10.5281/zenodo.21474351 (project-level).

\bibitem{FredriksenLean}
D.~E.~Fredriksen, \texttt{exotic-ccr-lean}, GitHub \texttt{Quantyra/exotic-ccr-lean}, tag \texttt{v0.1.1-collision},
\url{https://github.com/Quantyra/exotic-ccr-lean/releases/tag/v0.1.1-collision}
(Lean 4 formalization of the seed identities and dual-matrix algebra).
\end{thebibliography}
\end{document}
