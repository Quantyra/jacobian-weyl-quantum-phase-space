...
\begin{abstract}
We study the three-dimensional Keller map \(F\) announced by Alp\"oge (with Fable credited): \(\det DF\equiv -2\) and a three-point collision. Theorem~A restates those finite identities. Theorems~B--C record the standard cotangent/Piola/Weyl package for this seed; the analytic contribution is Theorems~D--F. The cotangent lift \(\Phi(q,p)=(F(q),J^{-T}p)\) is a polynomial Poisson map and non-injective. Dual fields \(X_j=\sum_k B_{jk}\partial_{q_k}\) commute and are divergence-free (Piola), so \(H_j=-iX_j\) are symmetric on \(C_c^\infty(\mathbb{R}^3)\). We define the induced polynomial Weyl endomorphism \(\psi\). The field \(X_1\) is incomplete via an explicit integral curve. In flow-box coordinates \((a,s,c)=F(q)\) one has \(X_1=\partial_s\) and \(\mathrm{d}q=\tfrac12\,\mathrm{d}a\,\mathrm{d}s\,\mathrm{d}c\). An open forward escape wall yields whole-orbit vectors in \(\ker(H^*+i)\) (transverse cutoffs only; no interior \(s\)-indicators); a measure-preserving sign involution transports them isometrically to \(\ker(H^*-i)\). Thus the Hilbert deficiency indices are
\[
(n_+,n_-)=(\aleph_0,\aleph_0),
\]
equivalently \((\infty,\infty)\) in conventional deficiency-index notation, for \(H=-iX_1\). Infinitely many self-adjoint extensions exist; the core algebraic CCR/Poisson data alone do not distinguish among them. The operator-theory mechanism is classical; the contribution is its explicit realization for this dual field. No gates, channels, computational advantage, or seed discovery claim. Paper-package candidate \textbf{v0.3.9-referee-revision} (proposed, untagged).
\end{abstract}
...
self-adjoint via a constructed weak minus-i witness. The theorem-source root is
\texttt{ff50f4a2a312591c2e5b26e71eb390ade9164b34}; the synchronized Lean
repository snapshot is
\texttt{94351f38d7d84fd073db14ff8764708fa9d2942b}. Neither is a final release
tag, and a new single-SHA Lean publication freeze remains pending. At the
theorem-source root the artifact proves
...
distinct such self-adjoint extensions exist~\cite{FredriksenLeanF}.

The paper's independent backward-wall proposition, exact extension cardinality,
...
We also record the boundary fact used when extending deficiency functions by
zero. Fix \(K\Subset W_+\) and suppose a sequence
\(\Psi_+(a_m,c_m,s_m)\) with \((a_m,c_m)\in K\) converges to a finite point
\(p\), after taking \((a_m,c_m)\to(a_*,c_*)\in K\). If
\(s_m\to\beta(a_*,c_*)\), the uniform wall estimate forces
\(\|\Psi_+(a_m,c_m,s_m)\|\to\infty\), a contradiction. If
\(\ell_+(a_*,c_*)<s_*<\beta(a_*,c_*)\), continuity of the flow gives
\(p=\Psi_+(a_*,c_*,s_*)\in\Omega_+\). Finally, a finite limit at
\(s_*=\ell_+(a_*,c_*)\) would, by openness of the maximal-flow domain and
continuous dependence on initial data, extend the selected orbit below its
maximal lower time. Thus no finite boundary of \(\Omega_+\) lies over
\(K\). This excludes a hidden surface contribution wherever a transverse
cutoff supported in \(K\) is nonzero.
\end{proof}
...
\(\operatorname{div}X_1=0\). There is no interior jump because \(u_-\) has no
artificial \(s\)-cutoff. There is also no finite jump across
\(\partial\Omega_+\): near the lateral boundary \(\chi\) vanishes on a
neighborhood, and the boundary conclusion of the saturated-sheet lemma
excludes finite boundary points over \(K\). Thus extending by zero introduces
no hidden surface distribution.
...
Apply Fubini first to the indicator of the open variable domain \(D_+\). For
each fixed \((a,c)\), integrate by parts on compact subintervals of
\((\ell_+(a,c),\beta(a,c))\), and then exhaust that particular maximal
interval. This fiber-first procedure makes no measurable choice of the lower
endpoint or of an exhaustion. At the upper endpoint the sheet escapes every
compact set, so \(\varphi\) vanishes eventually. At the lower endpoint, either
\(\ell_+\) is finite and maximality again forces spatial escape, or
\(\ell_+=-\infty\) and \(e^{s-\beta}\to0\). Hence both endpoint terms vanish.
The displayed majorant applies to the two bulk terms, so dominated convergence
passes to the improper limits and then through the transverse integral.
Consequently
\[
...
and existence of at least two distinct extension witnesses at the source root
identified above; the later synchronized snapshot is not yet a final
single-SHA release. The algebraic index
\texttt{standardDeficiency}\allowbreak\texttt{Index}, defined using
\texttt{Module.rank}, has lower bounds only.
The independent backward-wall construction, exact extension cardinality,
infinitely-many/continuum and algebraic-selection consequences,
arbitrary-operator theorem, strong CCR, and physical selection remain outside
the Lean corollary. No theorem-level dual-flow strong CCR obstruction is
...
Theorem F supplement: theorem-source root
ff50f4a2a312591c2e5b26e71eb390ade9164b34 and synchronized repository
snapshot 94351f38d7d84fd073db14ff8764708fa9d2942b (both untagged);
\path{ExoticCCR.theoremF},
\path{aleph0_le_standardDeficiencyIndex_X1}, and
\path{hilbertDeficiencyIndex_X1_eq_aleph0},
\texttt{theoremFVonNeumannClassification}, and
\texttt{theoremF\_exists\_two\_distinct\_selfAdjointExtensions}. Both standard
