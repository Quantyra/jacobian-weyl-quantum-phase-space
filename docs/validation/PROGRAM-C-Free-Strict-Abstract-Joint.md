...
**Date:** 2026-07-22  
**Status:** **Joint-Stone-Canonical = CONDITIONAL-OBSTRUCTION** (J2–J3); **Joint-Stone-Hom-1 = OBSTRUCT** (J4); **Joint-Stone-CP-1 = OBSTRUCT** (J5); **Unitary-Image-CP-1 = OBSTRUCT** (J6); **Diag-CP-Φ₀ = CONSTRUCT** (J6-C); Joint-Form split (Core **CONSTRUCT**, ESS-1 **OBSTRUCT**); full-\(\psi\) abstract \(C^*\) remains OPEN  
**Parent:** `PROGRAM-C-GenCP-Free-Strict.md`, `PROGRAM-C-C1d-Koopman-position.md`  
**Companion note:** C001 v0.8 cites this file for the joint \(q\)–\(p\) sector.  
**Residual detail:** `PROGRAM-C-residual-J4-Joint-Stone-Hom.md`, `PROGRAM-C-residual-mere-CP-Joint-Stone.md`, `PROGRAM-C-residual-CP-without-Stone.md`
...
J2 alone does **not** prove non-existence of every choice-dependent Joint-Stone package — only that **no canonical** package exists. **Later:** J4 kills Hom+Stone, **J5 kills mere unital normal CP+Stone** (Joint-Stone-CP-1), and **J6 kills unitary image of Schrödinger Stone unitaries without any SA-extension-of-\(H_1\) axiom** (Unitary-Image-CP-1). Non-Stone CP of \(\Phi_0\) on \(M\) **splits**: Diag-CP-Φ₀ **CONSTRUCT** (J6-C); full-\(\psi\) abstract \(C^*\) remains OPEN.
...
J5 kills CP **with** Stone. Unital normal CP extensions of \(\Phi_0\) **without** Stone / SA momentum implementation are handled by Theorem J6 / J6-C (not left as an undifferentiated OPEN blob).

### 5d. Theorem J6 — Unitary-Image-CP-1 obstruction; Diag-CP-Φ₀ construct
...
**Named pack Diag-CP-Φ₀.**  
\(\Phi_{\mathrm{diag}}=\Phi_0\circ E\) with \(E:M\to\mathcal{B}_b(Q)\) the normal conditional expectation onto positions.

**Theorem J6-C.** Diag-CP-Φ₀ is a unital normal CP extension of \(\Phi_0\) to \(M\) with \(\Phi_{\mathrm{diag}}(U(s))=0\) for \(s\neq 0\) (no Stone/SA momentum).

**Proof sketch (full write-up: `PROGRAM-C-residual-CP-without-Stone.md`).**  
J6: unitary image \(\Rightarrow\) multiplicative domain \(\Rightarrow\) dual-\(F_1\) covariance \(\Rightarrow\) J4 geometry. J6-C: composition of normal CP maps; crossed-product conditional expectation.
...
3. **No claim that every joint CP construction is impossible** — only Joint-Stone-Canonical (J2), full-triple Joint-Stone under G4 pairs (J3), multiplicative Joint-Stone-Hom-1 (J4), mere-CP Joint-Stone-CP-1 (J5), Unitary-Image-CP-1 (J6), and Joint-Form-ESS-1 (J4-E). Diag-CP-Φ₀ is constructed and is not a momentum channel.
4. **No claim beyond what is proved here:**  
   ...
   - full-\(\psi\) abstract \(C^*\) remains open; Joint-Form-Core is form-level only.
